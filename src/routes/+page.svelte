<script lang="ts">
  ...
  function copyCal() {
    navigator.clipboard.writeText(Math.round(totalCal).toString());
  ...
  function copyProt() {
    navigator.clipboard.writeText(Math.round(totalProt).toString());
...
<div class="p-4 max-w-md mx-auto">
      ...
      <button
        onclick={() => {
          if (confirm("Delete this meal?"))
            appState.deleteMeal(appState.activeCategory, appState.activeMealId);
        }}
  ...
  {#if activeMeal.items.length > 0}
    ...
    >
      ...
      ></div>
      <h3 class="text-sm font-bold text-muted mb-3 uppercase tracking-wider">
        Meal Summary
      </h3>
      <div class="flex justify-between items-end">
        ...
        >
          ...
          <div class="text-3xl font-black text-content leading-none">
            {Math.round(totalProt)}<span class="text-lg text-muted ml-1">g</span
            >
          </div>
        </button>
      </div>
